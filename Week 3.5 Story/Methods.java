...
public class Methods {
    ...
    static class Library {
        ...
        List<Book> search(String key, boolean exact) {
            ...
            for (Book b : books) {
                if (exact && b.title.equals(key)) res.add(b);
                else if (!exact && b.title.contains(key)) res.add(b);
            }
            return res;
        }
    }
    ...
    public static List<String> robotLabels(int[] w) {
        ...
        for (int i =0;i<w.length;i++) {
            if (w[i] % 15 == 0) res.add("Hazardous");
            else if (w[i] % 3 == 0) res.add("Heavy");
            else if (w[i] % 5 == 0) res.add("Fragile");
            else res.add(String.valueOf(w[i]));
        }
        return res;
    }
