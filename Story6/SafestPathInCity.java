 ...
 * @Description- This is to find a path from node start to node end such that the path safety is minimized
...
public class SafestPathInCity {
    ...
    public static int findSafestPath(int n, int[][] edges, int start, int end) {
        ...
        for (int[] edge : edges) {
            ...
            int to = edge[1];
            int risk = edge[2];
            adjacencyList[from].add(new int[]{to, risk});
            adjacencyList[to].add(new int[]{from, risk});
        }
